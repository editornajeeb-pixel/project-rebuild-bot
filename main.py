...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🥷 Jiraiya Sensei is ready.\n"
        "\"Small actions repeated daily become destiny.\"\n"
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in MAIN_MENU_BUTTONS:
        await update.message.reply_text(
            ...
            reply_markup=submenu(text)
        )
        return

    if text == BACK or text == "🔄 Refresh Menu":
        await update.message.reply_text(
            "Choose your next action below.",
            reply_markup=main_menu()
        )
    ...
    if text in {"📈 Week", "📈 Weekly Analytics"}:
    ...
    if text in {"📊 Status", "📊 Fitness Summary"}:
    ...
    if text == "🎯 Current Goal":
        await progress(update, context)
        return

    if text == "📊 Nutrition Summary":
        await foodlog(update, context)
        return

    if text in {"🔥 Workout Streak", "🔥 Streaks"}:
        await streak(update, context)
        return

    if text == "📚 Recall Memories":
        await memories(update, context)
        return
    ...
    if text in {"🧠 Daily Advice", "📊 Analyze Progress", "⚡ Coaching"}:
        await sensei(update, context)
        return
    ...
    if text == "✅ Habit Complete":
        await update.message.reply_text(
            "Use an existing habit command:\n"
            "/water 2\n"
            "/sleep 7\n"
            "/workout yes"
        )
    ...
    if text == "➕ Add Schedule":
        await setschedule(update, context)
        return

    if text == "📊 Monthly Analytics":
        await update.message.reply_text(
            "Monthly analytics is coming soon. Use /week for now."
        )
        return
...
def main():
    ...
    app.add_handler(MessageHandler(
        filters.Regex(
            "^(" + "|".join(re.escape(button) for button in MENU_BUTTONS) + ")$"
        ) & ~filters.COMMAND,
        menu_handler
    ))
