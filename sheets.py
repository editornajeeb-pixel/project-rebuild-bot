...
if not credentials_json:
    fallback_file = ROOT_DIR / "credentials.json"
    if fallback_file.exists():
        with open(fallback_file, "r", encoding="utf-8") as fp:
            credentials_json = fp.read()

if not credentials_json:
    raise RuntimeError(
        "Missing GOOGLE_CREDENTIALS_JSON. Set it in Railway environment variables or add a local credentials.json file."
    )
...
try:
    ...
    raise RuntimeError(
        "GOOGLE_CREDENTIALS_JSON is not valid JSON. Check your credentials value."
    ) from exc
